...
- `lib/db/exception-review-provider.ts` — `reviewTraceabilityExceptionWithProvider`,
  ...
  Batch A1-A5 provider tests — sufficient for the CRAP gate). Statement/branch coverage is **not**
...
- `lib/api/problem.ts` — the `rateLimitedResponse` (RFC 9457 `429` / `Retry-After`) builder,
  added to the snapshot when the Batch A5 rate-limit suite began exercising it.
- `lib/security/rate-limit.ts` — the `RateLimiter` checkpoint seam (`noopRateLimiter` /
  `FixtureFixedWindowRateLimiter`), exercised by the Batch A5 rate-limit suite.
...
Requires a disposable PostgreSQL test DB (the a1-a5 suites are provider-backed). Use
...
export TEST_DATABASE_URL="$DATABASE_URL"        # db name contains "test" → passes all provider fences (a1-a5)
npx drizzle-kit migrate --config drizzle.config.ts        # apply schema to a FRESH db

# 2. regenerate the snapshot: runs all provider suites (a1-a5) under coverage, converts V8 -> Istanbul
