...
- Batch A is accepted as of 2026-06-05 (`ops/deltas/0061-batch-a-acceptance.md`); the CI provider-DB database name was renamed `fsma204_a1a2_test` -> `fsma204_provider_test` (neutral; still satisfies every suite's `test`-token fence) and the truth surfaces were de-staled from "a1/a2/a3" to "a1-a5". Batch B is split B0 (DB-hardening on `batch-b0-db-hardening`) -> B1 (route wiring), per design 03-03 / delta 0062; B0 wires no route, so the split is as-safe as the prior bundle.
