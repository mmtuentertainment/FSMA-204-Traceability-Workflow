...
- Both routes go through `handleReadAction()` in `lib/api/route-boundary.ts`.
- Both routes use `fixtureMockRecallSource` in `lib/api/mock-recall-source.ts`, which gates access by the server-derived public fixture tenant.
- The Problem catalog in `lib/api/problem.ts` owns not-found, unauthorized, and forbidden Problem responses.
...
- `IdempotencyStore` and `AuditSink` are the future mutating-write integration points.
- Current defaults are public-fixture adapters only; they do not select or configure providers.
...
- The Batch 31 first mutating-write design recommends exception-review PATCH as the first real write, but implementation is still gated.
