...
- **Scaffold (configured, NOT route-wired): PostgreSQL via Drizzle ORM + `pg`.**
  ...
  - **Provider service** — `lib/db/exception-review-provider.ts` implements the full provider-backed exception-review write (tenant-membership RBAC, an optional per-action rate-limit checkpoint after RBAC and before the idempotency reserve, idempotency reserve/replay/conflict/reclaim, `FOR UPDATE` row locking, append-only audit insert, transaction wrapper). The rate-limit checkpoint is an internal seam, not an external integration: `ProviderExceptionReviewRequest.limiter?` (`lib/db/exception-review-provider.ts:78-80,590-603`) is an in-process `RateLimiter` (`lib/security/rate-limit.ts`, injected clock only, no new runtime dependency); when absent the path is byte-identical to the un-limited write. This is exercised only by the provider DB suites `tests/db/exception-review-provider-a1..a5.test.ts` against a real Postgres test DB — **never by a live route**.
...
| `TEST_DATABASE_URL` | `tests/db/exception-review-provider-a1..a5.test.ts`; `scripts/run-db-coverage.mjs:90` | Connection for the provider DB test suites and coverage regen; name must contain "test" (safety fence). | Tests / coverage only |
